...
class MailClient:
    ...
    def _load_banned_domains(self) -> set:
        """加载禁用域名列表（优先从面板 API 获取，失败则从本地文件读取）"""
        domains = set()

        # 优先从面板 API 获取
        if Config.PANEL_BASE:
            try:
                resp = requests.get(
                    f"{Config.PANEL_BASE}/api/v1/banned-domains",
                    headers={"User-Agent": "Mozilla/5.0"},
                    timeout=10,
                )
                if resp.status_code == 200:
                    data = resp.json()
                    api_domains = data.get("domains", [])
                    for d in api_domains:
                        if d:
                            domains.add(d.lower().strip())
                    if domains:
                        print(f"✅ 从面板 API 获取禁用域名: {len(domains)} 个")
                        return domains
            except Exception as e:
                print(f"⚠️ 从面板 API 获取禁用域名失败: {e}")
            ...
            try:
                with open(path, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        domains.add(line)
                if domains:
                    print(f"✅ 从本地文件获取禁用域名: {len(domains)} 个")
            except Exception as e:
...
class ChatGPTRegister:
    ...
    def register_one(self, thread_id: int = 0, mail_client: MailClient = None, enable_proxy_switch: bool = True) -> Optional[Dict]:
        ...
        try:
            ...
            account["_team_checkout_url"] = team_checkout_url
            ...
            if team_checkout_url:
                print(f"{prefix} 💳 Team 绑卡链接: {team_checkout_url[:60]}...")
    ...
    def _save_account(self, account: Dict):
        ...
        with self.save_lock:
            try:
                ...
                team_checkout_url = account.pop('_team_checkout_url', None)
                ...
                if checkout_url:
                    with open('chatgpt_accounts_check_url.txt', 'a', encoding='utf-8') as f:
                        f.write(f"{timestamp}｜{account['email']}｜Plus｜{checkout_url}\n")
                    print(f"💳 Plus 绑卡链接已保存到 chatgpt_accounts_check_url.txt")
                if team_checkout_url:
                    with open('chatgpt_accounts_check_url.txt', 'a', encoding='utf-8') as f:
                        f.write(f"{timestamp}｜{account['email']}｜Team｜{team_checkout_url}\n")
                    print(f"💳 Team 绑卡链接已保存到 chatgpt_accounts_check_url.txt")
            except Exception as e:
                print(f"⚠️ 保存账号失败: {e}")

        if Config.PANEL_IMPORT_ENABLED:
            self._import_to_panel(account, checkout_url, team_checkout_url)
    ...
    def _import_to_panel(self, account: Dict, checkout_url: Optional[str], team_checkout_url: Optional[str] = None):
        """导入账号到面板（适配 chatgpt-panel Go 后端）"""
        token = self._get_panel_token()
        if not token:
            return

        # Go 后端 API 接口：POST /api/v1/accounts/import
        # 使用 snake_case 命名法
        payload = {
        ...
        if account.get("cookies"):
            payload["cookies"] = account.get("cookies")

        if account.get("expired"):
            payload["expired"] = account.get("expired")

        if account.get("subscription_status"):
            payload["subscription_status"] = account.get("subscription_status")
            payload["type"] = account.get("subscription_status")

        try:
            resp = requests.post(
                f"{Config.PANEL_BASE}/api/v1/accounts/import",  # Go 后端路径
            ...
            if resp.status_code == 200:
                ...
                print("✅ 已导入面板")
                if result.get("id"):
                    print(f"   📋 账号ID: {result['id']}")
                if checkout_url:
                    print(f"   💳 Plus 绑卡链接: {checkout_url[:50]}...")
                if team_checkout_url:
                    print(f"   💳 Team 绑卡链接: {team_checkout_url[:50]}...")
            else:
